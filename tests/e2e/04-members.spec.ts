...
test.describe('Membros — CRUD', () => {
  ...
  test('TC-031 [+] Admin cria novo membro com pastoral vinculada', async ({ page }) => {
    ...
    await page.getByLabel(/nome completo/i).fill('Maria Cantora')
    await page.getByLabel(/e-mail/i).fill('maria@test.dev')
  ...
  test('TC-032 [-] Criar membro sem nome é bloqueado', async ({ page }) => {
    await page.getByRole('button', { name: /novo membro/i }).click()
    await page.getByRole('button', { name: /cadastrar/i }).click()
    // HTML5 required — formulário permanece aberto
  ...
  test('TC-034 [+] Admin desativa membro (soft-disable)', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /novo membro/i }).click()
    await page.getByLabel(/nome completo/i).fill('Temporário Desativar')
    await page.getByRole('button', { name: /cadastrar/i }).click()
    ...
    // Membro desativado não aparece na listagem padrão (sem filtro de inativos)
